...
{
	LCD_EN_PORT |=  (1 << LCD_EN_PIN);               // enable pin =1  (port on)
	_delay_us(1);                                                           // these delay values come from datasheet
	LCD_EN_PORT &= ~(1 << LCD_EN_PIN);                 // enable pin =0   (port off)
	_delay_us(50); 					// allow LCD to latch
}                                                                                 //btw on and off, enable pulse is sent

// this function sends a nibble to the data pins (b3..b0 -> D7..D4)
static void lcd_send_nibble(uint8_t nib)		//only in this c file, not affected on other files
{
...
void lcd_cmd(uint8_t c)
{
	...
	if (c == 0x01 || c == 0x02)     //in case we are sending clear command, we need delay(datasheet)
	_delay_ms(2);
}
